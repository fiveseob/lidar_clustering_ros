...
// pt_3d_array
    ...
    void objectSegmentation(){
        ...
        vg.setLeafSize(0.05f, 0.05f, 0.01f); // 0.01f is 1cm
    ...
    void filteringObject(){
        for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : objectClusters){
            ...
            if(maxPoint.z < 0.5 && (maxPoint.x-minPoint.x>0.1 || maxPoint.y-minPoint.y>0.2) && maxPoint.x-minPoint.x<9 && maxPoint.y-minPoint.y<9 && (maxPoint.x-minPoint.x)*(maxPoint.y-minPoint.y)<20 && sqrt(pow(maxPoint.x,2)+pow(maxPoint.y,2))<35){
        ...
        for (int i = 0; i < polygon_array.polygons.size(); i++){
            ...
            for (int k = 0; k < pt_3d_array.polygons.size(); k++){
                for(int l = 0; l < pt_3d_array.polygons[k].polygon.points.size(); l++){
                    ...
                    cout << Cam_X << endl;
                    cout <<"========="<<endl;
                    if (abs(abs(x_pt) - abs(Cam_X)) < 20 && abs(abs(y_pt) - abs(Cam_Y)) < 2) {
