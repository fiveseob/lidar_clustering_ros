...
// pt_3d_array
	...
	for (size_t j = 0; j < Horizon_SCAN; ++j){   // H 1024channel
            for (size_t i = 0; i < N_SCAN; ++i){     //  64channel
                ...
                if (i < groundScanInd){
                    ...
                    // if ((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < -1.0) || (isnan(angle)&& fullCloud->points[lowerInd].z < -1.0)){   // straight
                    if ((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < 0.5) || (isnan(angle)&& fullCloud->points[lowerInd].z < 0.5)){   // straight
                        ...
                        if ((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < 0.5) || (isnan(angle)&& fullCloud->points[lowerInd].z < 0.5)){   // straight       
                            ...
                            if (fullCloud->points[lowerInd].z < 2.0 && fullCloud->points[lowerInd].x < MAX_X && fullCloud->points[lowerInd].x > MIN_X && fullCloud->points[lowerInd].y > MIN_Y && fullCloud->points[lowerInd].y < MAX_Y)
    ...
    void filteringObject(){
        for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : objectClusters){
            ...
            if(maxPoint.z < 2.0 && ( maxPoint.x-minPoint.x > 0.05 || maxPoint.y-minPoint.y>0.02) && maxPoint.x-minPoint.x<4 && maxPoint.y-minPoint.y<4 && (maxPoint.x-minPoint.x)*(maxPoint.y-minPoint.y)<20 && sqrt(pow(maxPoint.x,2)+pow(maxPoint.y,2))<50){
                cout << "flag3" << endl;
