...
// pt_3d_array
    ...
    {
        // std::vector<vector<float>> mat_yaw;
        // std::vector<vector<float>> mat_pitch;
        // std::vector<vector<float>> mat_roll;
        Eigen::Matrix3f mat_yaw = Eigen::Matrix3f::Identity();
        Eigen::Matrix3f mat_pitch = Eigen::Matrix3f::Identity();
        Eigen::Matrix3f mat_roll = Eigen::Matrix3f::Identity();


        mat_yaw << cos(cur_imu[2]), -sin(cur_imu[2]), 0,
                   sin(cur_imu[2]), cos(cur_imu[2]), 0,
                   0 , 0 , 1;

        mat_pitch << cos(cur_imu[1]), 0, sin(cur_imu[1]),
                     0, 1, 0,
                     -sin(cur_imu[1]), 0, cos(cur_imu[1]);

        mat_roll << 1, 0, 0,
                     0, cos(cur_imu[0]), -sin(cur_imu[0]),
                     0, sin(cur_imu[1]), cos(cur_imu[1]);


        rotation_mat = mat_pitch;

        cout << mat_pitch << endl;

        // for ( int i = 0; i < mat_pitch.size(); i++){
        //     int count = 0;
        //     for ( int j = 0; j < mat_pitch[0].size(); j++){
        //         cout << mat_pitch[i][j] << "";
        //         count +=1;
        //         if (count == 3){
        //             cout << endl;
    ...
    void groundRemoval(){
        ...
        // if ( rad2deg < -0.9 ){         // LookDown   // YEONGJONG = -2, SONGDO = -0.9
        //     // MAX_RM + 1.3;
        //     // MAX_CL + 2.8;
        //     MAX_RM = 0.5;
        //     MAX_CL = 2.0;
        ...
        MAX_RM = 0.5;
        MAX_CL = 2.0;
        // else if ( rad2deg > 0.7 ){       // LookUp   // YEONGJONG = 1.4, SONGDO = 0.7
        //     // MAX_RM - 0.7;  
        //     // MAX_CL + 1.1;
        //     MAX_RM = -1.5;
        //     MAX_CL = 0.3;
	...
	for (size_t j = 0; j < Horizon_SCAN; ++j){   // H 1024channel
            for (size_t i = 0; i < N_SCAN; ++i){     //  64channel
                ...
                if (i < groundScanInd){
                    ...
                    if ((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < MAX_RM) || (isnan(angle)&& fullCloud->points[lowerInd].z < MAX_RM)){
                        groundCloud->push_back(fullCloud->points[lowerInd]);
                        if (fullCloud->points[lowerInd].x < -3 && fullCloud->points[lowerInd].x > -60 && fullCloud->points[lowerInd].y > -3 && fullCloud->points[lowerInd].y < 3){
                        ...
                        if((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < MAX_RM) || (isnan(angle)&& fullCloud->points[lowerInd].z < MAX_RM)){
                            ...
                            if (fullCloud->points[lowerInd].z < MAX_CL && fullCloud->points[lowerInd].x < MAX_X && fullCloud->points[lowerInd].x > MIN_X && fullCloud->points[lowerInd].y > MIN_Y && fullCloud->points[lowerInd].y < MAX_Y)
