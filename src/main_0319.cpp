...
// pt_3d_array
    ...
    void degreeCallback(geometry_msgs::Transform msg){
        rad2deg = (msg.rotation.y/M_PI)*180.0;
    ...
    void filteringObject(){
        for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : objectClusters){
            ...
            if(maxPoint.z < MAX_CL && ( maxPoint.x-minPoint.x > 0.08 || maxPoint.y-minPoint.y>0.05) && maxPoint.x-minPoint.x<4 && maxPoint.y-minPoint.y<4 && (maxPoint.x-minPoint.x)*(maxPoint.y-minPoint.y)<20 && sqrt(pow(maxPoint.x,2)+pow(maxPoint.y,2))<50){
