...
// pt_3d_array
	...
	int tunnel_flag = 0;
        ...
        cout << "--------------------------------------" << ImuDegree*180.0/M_PI << endl;
		...
		if (fullCloud->points[lowerInd].x < -10 && fullCloud->points[lowerInd].x > -15 && fullCloud->points[lowerInd].y < 2 && fullCloud->points[lowerInd].y > -2 && fullCloud->points[lowerInd].z > 2.3){
			...
			if(fullCloud->points[lowerInd].x < tunnel_distance){
                ...
                if (i < groundScanInd){
                    ...
                    if ((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < -0.7) || (isnan(angle)&& fullCloud->points[lowerInd].z < -0.7)){
                        ...
                        if((abs(angle - sensorMountAngle) <= groundThreshold && fullCloud->points[lowerInd].z < -0.7) || (isnan(angle)&& fullCloud->points[lowerInd].z < -0.7)){
                            ...
                            if (fullCloud->points[lowerInd].z < 1.0 && fullCloud->points[lowerInd].x < MAX_X && fullCloud->points[lowerInd].x > MIN_X && fullCloud->points[lowerInd].y > MIN_Y && fullCloud->points[lowerInd].y < MAX_Y)
    ...
    void filteringObject(){
        for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : objectClusters){
            ...
            if(maxPoint.z < 0.5 && (maxPoint.x-minPoint.x>0.6 || maxPoint.y-minPoint.y>0.6) && maxPoint.x-minPoint.x<9 && maxPoint.y-minPoint.y<9 && (maxPoint.x-minPoint.x)*(maxPoint.y-minPoint.y)<20 && sqrt(pow(maxPoint.x,2)+pow(maxPoint.y,2))<35){
    ...
    void cal_point(){
        ...
        for (int i = 0; i < polygon_array.polygons.size(); i++){
            ...
            for(int j = 0; j < polygon_array.polygons[i].polygon.points.size(); j++){
                ...
                if (x_pt > 4.0){
                    x_pt = min(x_pt, polygon_array.polygons[i].polygon.points[j].x);
                }
                if (z_pt > -1.8){
            ...
            for (int k = 0; k < pt_3d_array.polygons.size(); k++){
                for(int l = 0; l < pt_3d_array.polygons[k].polygon.points.size(); l++){
                    ...
                    if (abs(abs(x_pt) - abs(Cam_X)) < 10 && abs(abs(y_pt) - abs(Cam_Y)) < 2) {
